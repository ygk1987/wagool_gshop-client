      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(item, index) in cartList" :key="item.id">
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins" @click="changeItemCunt(item, -1)">-</a>
            <!-- <input autocomplete="off" type="text" class="itxt" v-model="item.skuNum"> -->
            <input autocomplete="off" type="text" class="itxt" 
            :value="item.skuNum" @change="changeItemCunt(item, $event.target.value * 1 - item.skuNum)">
            <a href="javascript:void(0)" class="plus" @click="changeItemCunt(item, 1)">+</a>
          </li>
...
<script>
  ...
  export default {
    ...
    methods:{
      //修改某个购物项的数量
      async changeItemCunt(item, changeNum){
        const {skuId} = item
        //最终的数量必须大于0
      ...
      async checkAll(event){
        ...
        const isChecked = event.target.checked * 1 //将布尔值转换为number
        //将所有购物项都更新为isChecked
        // const promises = [];
      ...
      checkCartItem(item, event){
        // const isChecked = item.target.checked 也可以
        //得到新的勾选状态
