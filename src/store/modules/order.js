...
const mutations = {
  ...
  RECEIVE_TRANDE_INFO(state, {tradeInfo}){ //从对象中解构出来需要的数据
...
const actions = {
  ...
  async getTradeInfo({commit}){
    ...
    if(result.code === 200){
      ...
      commit('RECEIVE_TRANDE_INFO', {tradeInfo});//传递给mutations的总是包含要传递数据的对象
    }
  },
  ...
  async getPayInfo({commit}, orderId){
    ...
    if(result.code === 200){
      ...
      commit('RECEIVE_PAY_INFO', {payInfo}) //传递给mutations的总是包含要传递数据的对象
    }
  }
